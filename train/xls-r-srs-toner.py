...
# -*- coding: utf-8 -*-
#
# Fine-tune an XLS-R-wav2vec2 model to recognize tone and vowel length in
# Tsuut'ina.
#
# See:
#
...
srs_model = "wav2vec2-xls-r-300-srs-tones"
...
# Load the Tsuut'ina training and test data sets from the given TSV files,
# remove the object packaging to get at just the primary data, then process
# each set to produce orthographic sentences in 'sentence' and a list of
# phonemes in 'phonemes' (with "|" as the word separator in both).
...
# Filter out any rows in the training data that don't contain tone (e.g.,
# hesitations that didn't end in a vowel, and hence lead to "tones" being
# an empty string -- which causes later training/evaluation processes to
# fail).
srs_train_data = srs_train_data.filter(lambda r: len(r['tones']) > 0)
srs_valid_data = srs_valid_data.filter(lambda r: len(r['tones']) > 0)

def extract_all_chars(batch):
#    all_text = "|".join(batch["sentence"])
    all_text = "|".join(batch["tones"])
...
model = transformers.Wav2Vec2ForCTC.from_pretrained(
    ...
    mask_time_length = 5,       # ** CDC: added 2022-03-22 (default: 10)
...
# to the trainer to do the actual model training.
